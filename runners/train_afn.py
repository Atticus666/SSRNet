...
from models import AFN, AFNTrainer
...
class AFNExperiment:
    """
    Main experiment class for AFN training and evaluation using Keras fit.
    """
    ...
    def _create_model(self, feature_size: int, field_size: int) -> AFN:
        """
        Create AFN model with specified parameters.
        ...
        Returns:
            Configured AFN model
        """
        model = AFN(
            feature_size=feature_size,
            field_size=field_size,
            embedding_size=self.args.embedding_size,
            logarithmic_neurons=self.args.logarithmic_neurons,
            afn_hidden_units=self.args.afn_hidden_units,
            afn_activation=self.args.afn_activation,
            afn_dropout=self.args.afn_dropout,
            ensemble_dnn=self.args.ensemble_dnn > 0,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='AFN Training')
    ...
    parser.add_argument('--logarithmic_neurons', type=int, default=5, 
                        help='Number of logarithmic neurons (adaptive order)')
    parser.add_argument('--afn_hidden_units', type=int, nargs='+', default=[64, 64, 64], 
                        help='AFN network hidden layer sizes')
    parser.add_argument('--afn_activation', type=str, default='relu', 
                        help='Activation function for AFN network')
    parser.add_argument('--afn_dropout', type=float, default=0.0, 
                        help='Dropout rate for AFN network')
    parser.add_argument('--ensemble_dnn', type=int, default=1, 
                        help='Use ensemble DNN (1=True, 0=False)')
    parser.add_argument('--dnn_hidden_units', type=int, nargs='+', default=[64, 64, 64], 
                        help='DNN network hidden layer sizes')
    parser.add_argument('--dnn_activation', type=str, default='relu', 
                        help='Activation function for DNN network')
    parser.add_argument('--dnn_dropout', type=float, default=0.0, 
                        help='Dropout rate for DNN network')
    ...
    # Other parameters
    parser.add_argument('--save_path', type=str, default='../checkpoint/afn', help='Save path')
...
def main():
    ...
    # Print configuration
    print("\n" + "="*80)
    print("AFN Training Configuration")
    print("="*80)
    print(f"Dataset: {args.data}")
    print(f"Embedding Size: {args.embedding_size}")
    print(f"Logarithmic Neurons: {args.logarithmic_neurons}")
    print(f"AFN Hidden Units: {args.afn_hidden_units}")
    print(f"AFN Activation: {args.afn_activation}")
    print(f"AFN Dropout: {args.afn_dropout}")
    print(f"Ensemble DNN: {bool(args.ensemble_dnn)}")
    if args.ensemble_dnn:
        print(f"DNN Hidden Units: {args.dnn_hidden_units}")
        print(f"DNN Activation: {args.dnn_activation}")
