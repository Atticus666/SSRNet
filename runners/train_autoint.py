...
from models import AutoInt, AutoIntTrainer
...
class AutoIntExperiment:
    """
    Main experiment class for AutoInt training and evaluation using Keras fit.
    """
    ...
    def _create_model(self, feature_size: int, field_size: int) -> AutoInt:
        """
        Create AutoInt model with specified parameters.
        ...
        Returns:
            Configured AutoInt model
        """
        model = AutoInt(
            feature_size=feature_size,
            field_size=field_size,
            embedding_size=self.args.embedding_size,
            attention_blocks=self.args.blocks,
            attention_heads=self.args.heads,
            block_shape=self.args.block_shape,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='AutoInt Training with Keras fit')
    ...
    parser.add_argument('--block_shape', nargs='+', type=int, default=[64, 64, 64],
                       help='Output dimensions for each block (comma-separated)')
    ...
    parser.add_argument('--deep_layers', nargs='+', type=int, default=[], help='Optional Deep layers')
    parser.add_argument('--dropout_rates', nargs='+', type=float, default=[0.0, 0.0, 0.0],
                       help='Dropout rates for attention,embedding,deep (comma-separated)')
    ...
    parser.add_argument('--save_path', type=str, default='./models/autoint_keras_experiment/',
    ...
    # args.block_shape = [int(x) for x in args.block_shape.split(',')]
    # args.dropout_rates = [float(x) for x in args.dropout_rates.split(',')]
    
    # Print configuration
    if args.verbose > 0:
        print("AutoInt Training Configuration (Keras fit):")
        print("="*50)
    ...
    # Run experiments
    experiment = AutoIntExperiment(args)
