...
from models import RankMixer, RankMixerTrainer
...
class RankMixerExperiment:
    """
    Main experiment class for RankMixer training and evaluation using Keras fit.
    """
    ...
    def _create_model(self, feature_size: int, field_size: int) -> RankMixer:
        """
        Create RankMixer model with specified parameters.
        ...
        for epoch in range(self.args.epoch):
            ...
            if self.args.verbose > 0:
                parts_iter = tqdm(parts_range, 
                                desc=f"Epoch {epoch + 1}/{self.args.epoch} => Parts", 
                                unit="part",
                                leave=False)
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RankMixer Training with Keras fit')
    ...
    parser.add_argument('--hidden_dim', type=int, default=256,
                       help='Hidden dimension for each token')
    parser.add_argument('--num_blocks', type=int, default=2,
                       help='Number of RankMixer blocks')
    ...
    parser.add_argument('--save_path', type=str, default='./models/rankmixer_keras_experiment/',
    ...
    # Update save_path with timestamp
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    args.save_path = os.path.join(args.save_path, f"{args.data.lower()}_{timestamp}")
    
    # Print configuration
    if args.verbose > 0:
        print("RankMixer Training Configuration (Keras fit):")
        print("="*50)
    ...
    # Run experiments
    experiment = RankMixerExperiment(args)
